...
@cuda.jit
def _power_gradient_numba_kernel(a, output, power):
    ...
    if row < a.shape[0] and col < a.shape[1]:
        output[row, col] = power * a[row, col] ** (power - 1)


def power_gradient_numba(a, power):
    ...
    _power_gradient_numba_kernel[blocks_per_grid, threads_per_block](a, output, power)
    return output
